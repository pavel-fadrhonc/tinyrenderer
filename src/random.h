...
namespace Random
{
	...
	static inline R get(S min, T max)
	{
		if constexpr ((std::is_same_v<R, S> && std::is_same_v<S, T> && std::is_same_v<R, float>) ||
			(std::is_same_v<R, S> && std::is_same_v<S, T> && std::is_same_v<R, double>))
		{
			constexpr R diff = max - min;
			return static_cast<R>(get(0ui32, UINT32_MAX)) / static_cast<R>(UINT32_MAX) * diff + min;
		}
		else
	...
	static inline float get(float min, float max)
	{
		...
		return static_cast<float>(get(0ui32, UINT32_MAX)) / static_cast<float>(UINT32_MAX) * diff + min;
	}
}
