...
Vec3f BasicScreenSpace::vertex(u32 faceIdx, u8 vertIdx)
{
	...
	Vec3f vertexNormalNDC = (mgl::MVP_IT * vertexNormal.ToDirection()).ToVec3();
	...
	SetVaryingData3(VERTEX_VIEWSPACE_VARYING_DATA_HASH, vertIdx, (mgl::ViewMat * positionWS).FromHomogeneous());
...
bool NormalMappedPhongFragmentShader::fragment()
{
	...
	const Vec3f lightNDC = (mgl::ViewMat * mgl::LightDir.ToDirection()).ToVec3().normalize();
	//std::cout << normal << '\n';
	...
	const Vec3f viewNDC = (mgl::ViewMat * viewWS.ToDirection()).ToVec3().normalize();
